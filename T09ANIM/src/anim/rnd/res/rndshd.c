 ...
 * RETURNS: None.
 */
static VOID VD6_RndShdLog( CHAR *FileNamePrefix, CHAR *ShaderName, CHAR *Text )
{
  FILE *F;

  if ((F = fopen("bin/shaders/shd{30}vd6.log", "a")) == NULL)
    return;
  fprintf(F, "%s : %s\n%s\n\n", FileNamePrefix, ShaderName, Text);
  fclose(F);
} /* End of 'VD6_RndLoadTextFromFile' function */
 ...
 *   (CHAR *) load text.
 */
static CHAR * VD6_RndLoadTextFromFile( CHAR *FileName )
{
  FILE *F;
  INT flen;
  CHAR *txt;
 ...
 */
static UINT VD6_RndShdLoad( CHAR *FileNamePrefix )
{
  CHAR *txt;
  struct
  {
  ...
  for (i = 0; i < NoofS; i++)
  {
    ...
    txt = VD6_RndLoadTextFromFile(Buf);
    if (txt == NULL)
    {
      VD6_RndShdLog(FileNamePrefix, shd[i].Name, "Error load file");
      is_ok = FALSE;
      break;
    }
    /* Create shader */
    ...
    if (shd[i].Id == 0)
    {
      VD6_RndShdLog(FileNamePrefix, shd[i].Name, "Error create shader");
      is_ok = FALSE;
      break;
    }
    ...
    if (res != 1)
    {
      ...
      VD6_RndShdLog(FileNamePrefix, shd[i].Name, Buf);
      is_ok = FALSE;
      break;
    }
  }
  ...
  if (is_ok)
    ...
    {
      VD6_RndShdLog(FileNamePrefix, "PROG", "Error create program");
      is_ok = FALSE;
    }
    else
    {
      ...
      if (res != 1)
      {
        ...
        VD6_RndShdLog(FileNamePrefix, "PROG", Buf);
        is_ok = FALSE;
      }
    }
  ...
  return prg;
} /* End of 'VD6_RndShdLoad' function */
 ...
 * RETUNS: None.
 */
static VOID VD6_RndShdFree( UINT ProgId )
{
  INT shds[5], n, i;
...
} /* End of 'VD6_RndShdFree' function */
...
{
  VD6_RndProgId = VD6_RndShdLoad("default");
} /* End of 'VD6_RndResInit' function */
...
} /* End of 'VD6_RndShdUpdate' function */
